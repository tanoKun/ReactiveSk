...
class NonSuspendCallFunctionEffect : Effect() {
    ...
    override fun toString(e: Event?, debug: Boolean): String = "call function with ${argumentExprs.map { it.toString(e, debug) }.reduceOrNull { acc, s -> "$acc, $s" }} in ${targetExpr.toString(e, debug)}"
}
